...
- **More satellite basemaps** — Sentinel-2 (date slider), OSM context, user-keyed Bing (grows Satellite Compare). ✅ User-keyed **Mapbox & Google** basemaps + API-keys settings tab + monthly usage counters are speced in [KEYED_PROVIDERS.md](KEYED_PROVIDERS.md) (with the per-provider capture/cache legal rules).
