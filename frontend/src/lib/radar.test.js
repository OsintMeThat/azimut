...
import { RADAR_ID, onTrack, orbitMark, passLabel, passOf, radarId, sameTrack, validPass } from './radar.js';

describe('a Sentinel-1 pass on the provider id', () => {
  it('rides as a day and a time without colons, and comes back', () => {
    const pass = { date: '2026-05-14', time: '05:42:10' };
    expect(radarId(RADAR_ID, pass)).toBe('sentinel1~2026-05-14~054210');
    expect(passOf('sentinel1~2026-05-14~054210')).toEqual(pass);
  });
  ...
  it('stays the plain id for the most recent pass, and on any other basemap', () => {
    ...
    expect(passOf('sentinel1')).toBe(null);
    expect(passOf('sentinel1~2026-05-14~99')).toBe(null);
